...
double* soma_vetorial(double coord[3], double k, double h, double c, int j)
{
...
double RuKuNew3 (double* (*Fovm)(double, double[3], double[3]), double h, double t0, double x0[3], double v0[3])
{
  ...
  double t1;
  //double *p1, *p2, *p3, *p4; //  ponteiros auxiliares
   ...
   for (int i = 0; i < 3; i++)
    {
      ...
      k1_v[i] = (*Fovm)(t0, soma_vetorial(x0, 0, h, 0, i), soma_vetorial(v0, 0, h, 0, i));
      
      k2_x[i] = v0[i] + (0.5) * h * k1_v[i];
      //p2 = (*Fovm)(t0 + 0.5 * h, soma_vetorial(x0, k1_x[i], h, 0.5, i), soma_vetorial(v0, k1_v[i], h, 0.5, i));
      k2_v[i] = (*Fovm)(t0 + 0.5 * h, soma_vetorial(x0, k1_x[i], h, 0.5, i), soma_vetorial(v0, k1_v[i], h, 0.5, i));
      
      k3_x[i] = v0[i] + (0.5) * h * k2_v[i];
      //p3 = (*Fovm)(t0 + 0.5 * h,soma_vetorial(x0, k2_x[i], h, 0.5, i),soma_vetorial(v0, k2_v[i], h, 0.5, i));
      k3_v[i] = (*Fovm)(t0 + 0.5 * h,soma_vetorial(x0, k2_x[i], h, 0.5, i),soma_vetorial(v0, k2_v[i], h, 0.5, i));
      
      k4_x[i] = v0[i] + h * k3_v[i];
      //p4 = (*Fovm)(t0 + h, soma_vetorial(x0, k3_x[i], h, 1, i), soma_vetorial(v0, k3_v[i], h, 1, i));
      k4_v[i] = (*Fovm)(t0 + h, soma_vetorial(x0, k3_x[i], h, 1, i), soma_vetorial(v0, k3_v[i], h, 1, i));
