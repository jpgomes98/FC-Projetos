...
Matrix eigenspace(Matrix& M, Matrix& eigenvalues, const Matrix& guess, int stop)
{
  ...
  double eigen = 0;
  double eps = 1e-13; // critério de paragem 
  double dif = 10;
  ...
  cout << "Verificando se a matriz é hermítica..." << endl;
  if (M.isSymmetric() && ((eigenvalues.ncol() == 1))){
    cout << "A matriz é hermítica. É possível encontrar uma base própria.\n\n";
	...
	// atualizando a variável que permite parar o ciclo
    ...
    /* Calcular os valores próprios de M */
    for (int i = 1; i <= stop; i++){
...
Matrix solveSchrodinger(double (&V_x)(double, double, double, double), Matrix& Energy, Matrix& X, int npos, double lim, int stop, istream& parametros, ofstream& outMatrix)
{
  ...
  for (int i = 1; i <= H.nlin(); i++){
	...
	H.set(i, i, 2 * par + V_x(a, b, k, X(i, 1)));
      }
      else{
	H.set(i, j, (-1) * par);
      }
  ...
  /* É possível o utilizador especificar quantas soluções quer calcular */
