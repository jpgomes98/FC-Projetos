...
int main()
{
  ...
  for (int i = 0; i < n_max; i++)
    {
      raio = sqrt((mat[i][0] * mat[i][0]) + (mat[i][1] * mat[i][1])); // Cálculo do raio 
      ...
      else
	{
      ...
      if ((i % 10 == 0) && (i > 0))
	{
	  pi = 4 * ((double) ni / (double) i);
	  piOut << i << " " << pi << endl;

	  /* Cálculo dos vários valores de pi a cada acréscimo do número de iterações em 10 
	     Imprime também esse valores para um ficheiro para fazer um plot do valor calculado
